...
1. Create a new branch and a new environment (under "Environments" in the repository settings) with the same name in the repository. The names must match since the deployment job gets the environment secrets by the branch name.
...
4. Add these repository secrets (Security / Secrets and variables / Actions in the repository settings).
   ...
   - `DEPLOY_SERVER_USERNAME` - the system user name of the stand;
